...
class FlowPolicy(nn.Module):
    def __init__(
        ...
        input_dim = state_dim + time_dim + action_dim
        ...
        self.vel_head = nn.Linear(hidden_dim, action_dim)
        ...
        torch.nn.init.uniform_(self.vel_head.weight, -1e-3, 1e-3)
        torch.nn.init.uniform_(self.vel_head.bias, -1e-3, 1e-3)
    ...
    def forward(
        ...
        # print(f"t_embed shape: {t_embed.shape},t : {t} ,t_embed: {t_embed}")
        # import pdb; pdb.set_trace()
        hidden = torch.cat([state, action, t_embed], dim=-1)
    ...
    def Euler(self, state: torch.Tensor, deterministic: bool = False, n_steps: Optional[int] = None, need_log_prob: bool = False):
        n_steps = self.n_steps
        dt = 1.0 / n_steps
        z = torch.randn(state.shape[0], self.action_dim, device = state.device)
        for step in range(n_steps):
            t = torch.ones(state.shape[0]) * step * dt
            t = t.unsqueeze(-1)
            # print(f"t shape: {t.shape}")
            # import pdb; pdb.set_trace()
            v = self.forward(state, z, t, deterministic, need_log_prob)
            z += v * dt
        return z
    
def ensure_2d(x: torch.Tensor) -> torch.Tensor:
    if x.ndim == 1:   # torch.Tensor 用 .ndim，numpy.ndarray 也是 .ndim
        x = x.unsqueeze(0)
    return x
...
class OneStepFlowPolicy(nn.Module):
    ...
    def forward(
        ...
        # hidden = self.trunk(state)
        t = torch.ones(state.shape[0])
        t = t.to(state.device)
        t_embed = self.time_embed(t.unsqueeze(-1))
        action = torch.randn(state.shape[0], self.action_dim, device=state.device)
        # print(f"state shape: {state.shape}, action shape: {action.shape}, t_embed shape: {t_embed.shape}")
        # import pdb; pdb.set_trace()
        hidden = torch.cat([state, action, t_embed], dim=-1)
        hidden = self.trunk(hidden)
        vel = self.vel_head(hidden)
...
class FQL:
    def __init__(
        ...
        flowpolicy_optimizer: torch.optim.Optimizer,
        ...
        self.flowpolicy_optimizer = flowpolicy_optimizer
    ...
    #flow_matching loss
    def _flowpolicy_loss(self, state: torch.Tensor, action: torch.Tensor) -> torch.Tensor:
        batch_size = state.shape[0]
        t = torch.rand(batch_size, device = self.device)
        t = t.unsqueeze(-1)
        z = torch.randn_like(action)
        action_t = t * action +(1 - t) * z
        v_t = action - z
        predict_v = self.flowpolicy(state, action_t, t)
        loss = F.mse_loss(predict_v, v_t)
        return loss
    
    #TBD (mean std) distill_loss
    def _distill_loss(self, state: torch.Tensor) -> torch.Tensor:
        with torch.no_grad():
            action = self.flowpolicy.Euler(state, need_log_prob=False)

        distill_action = self.actor(state, need_log_prob=False)
        loss = F.mse_loss(distill_action, action)
        return loss
    
    def _actor_loss(self, state: torch.Tensor) -> Tuple[torch.Tensor, float, float]:
        action = self.actor(state, need_log_prob=True)
    ...
    def update(self, batch: TensorBatch) -> Dict[str, float]:
        ...
        # self.alpha = self.log_alpha.exp().detach()
        #flow policy update
        flow_loss = self._flowpolicy_loss(state, action)
        self.flowpolicy_optimizer.zero_grad()
        flow_loss.backward()
        ...
        update_info = {
            ...
            "onestep_flow_loss": actor_loss.item(),
...
@torch.no_grad()
def eval_actor(
    env: gym.Env, actor: OneStepFlowPolicy, device: str, n_episodes: int, seed: int
) -> np.ndarray:
    env.seed(seed)
    actor.eval()
    episode_rewards = []
...
def train(config: TrainConfig):
    ...
    # Actor & Critic setup
    actor = OneStepFlowPolicy(state_dim, action_dim, config.hidden_dim, config.max_action)
    ...
    flowpolicy = FlowPolicy(state_dim, action_dim, config.hidden_dim, config.max_action)
    flowpolicy.to(config.device)
    flowpolicy_optimizer = torch.optim.Adam(
        flowpolicy.parameters(), lr=config.actor_learning_rate
    )
